...
- 🔗 **Seamless integration** of external IT systems into ChurchTools’ user rights and management through a simple [REST API](./backend/README.md) to access the encrypted passwords. This enables scenarios such as personal WiFi access control via the [ct‑radius project](https://github.com/lub90/ct-radius) and other challenge‑response based authentication systems (e.g., samba drives).
...
- The **CtPassStore extension** provides the user interface and stores the encrypted secondary passwords in its key‑value store.  
...
1. The CtPassStore ChurchTools extension offers a GUI where users can manage their secondary password.  
   ...
   - Setting or resetting a password may require the user’s primary ChurchTools password (unless admins configure otherwise).  
2. The chosen or generated secondary password is sent to the PHP backend, where it is **asymmetrically encrypted**.  
3. The encrypted password is then stored back in the CtPassStore ChurchTools Extension’s key‑value store.  
